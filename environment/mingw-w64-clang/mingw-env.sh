...
[[ $USE_COMPILER_WRAPPERS ]] && \
  ...
  cross_clang_flags="-rtlib=compiler-rt -fuse-ld=lld $_mguard -target $_target -Xclang -triple -Xclang $_target"
default_mingw_pp_flags=""
default_mingw_compiler_flags="$cross_clang_flags $default_mingw_pp_flags -O2 -pipe -fexceptions --param=ssp-buffer-size=4 -Wformat -Werror=format-security $_mguard"
